...
use crate::{
    ...
    load_model_profile, load_rewrite_parallelism, mark_empty_source_chapters_skipped,
...
pub(crate) async fn start_analysis(
    ...
    let job = create_job(&state, &novel_id, "analysis", total)?;
    ...
    match operation {
        ...
        Err(error) => {
            finalize_standalone_job_failure(&state, &novel_id, &job, &chapters, "analysis", &error)
        }
    }
}
