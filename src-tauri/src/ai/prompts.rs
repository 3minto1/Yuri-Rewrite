...
12. 女性化细节应覆盖正文中与主角有关的视线、评价、互动距离和社会称呼；标题仅按第 3 条的明确条件修改。新增内容必须服务当前剧情，不得破坏原文战力、伏笔、人物性格和逻辑。
...
13. marker 中的 index 是内部顺序标识，不是原标题中的章节编号。序章、楔子、番外等会使二者不同；不得据此修改标题或判定章节矛盾。
