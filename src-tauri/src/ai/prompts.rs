...
4. 原文中明显不属于小说正文的作者更新提示、求票求收藏、简短勘误、作者与读者互动、装饰分隔线和孤立乱码可以删除；完本感言、卷末后记、正式后记、番外及实际剧情正文不得按此规则删除。拿不准是否属于正文时必须保留。
5. 清除所有原男性主角痕迹，包括姓名、代词、身体描述、外貌气质、社会称呼、动作习惯、旁人称谓和亲密互动中的性别暗示；所有相关内容都要自然转换为女性主角表达。
6. 主角姓名和指定 NPC 姓名必须严格使用一致性资产中的“姓名映射表”。没有映射时才按同音或近音原则女性化，优先保留姓氏；例如萧炎改为萧妍，李火旺改为李火婉。
7. 仅与主角原名共享单字的未指定 NPC 不是主角，不得套用主角改名。例如主角“石昊”改为“石念昔”时，NPC“秦昊”如未被指定或映射，必须继续保留为“秦昊”。
8. 涉及同名、姓名来源、名字含义、旧名对比、以某字为名等句子，必须随主角新姓名重写逻辑；不得保留“她原本同名”“仍以旧名某字为名”等暴露旧主角姓名或与新姓名矛盾的表达。
9. 按基本设定中的身材和体型调整外貌、动作和互动细节，不要出现与设定冲突的描写。
10. 只有主角、用户指定的额外女性化人物、以及姓名映射表中明确存在映射的人物可以性别转换；其他配角、敌人、长辈、师父、兄弟、父亲、旁观者必须保持原文性别、身份、称谓和人称代词，不得跨章节忽男忽女。
11. 对未指定性转的人物，原文男性继续使用男性代词/称谓，原文女性继续使用女性代词/称谓，性别不明者保持原文称谓或中性表达，等原文或一致性资产明确后再固定。
12. 复数群体代词必须按成员构成判断：主角与男性角色共同被指代，或群体中包含任何未指定性转的男性成员时，必须使用“他们”或准确的群体称呼，禁止改成“她们”；只有确认全员女性时才能使用“她们”。性别构成不明时保留原文“他们”或使用中性群体称呼。
13. 对原文未明确性别或性别模糊的动物、灵兽、妖兽、凶兽、神兽、器灵等非人生物，保留原文中的人称代词和称谓，不要强行女性化或男性化。
14. 人物外貌特征必须前后一致。发色、瞳色、身高、体型、胸部设定、年龄感、标志性服饰、伤痕、气质和能力状态一旦由原文、设定或一致性资产确立，后续章节不得随意改变；例如上一章是金发，下一章不能无理由变成红发。
15. 如果原文没有明确外貌，不要每章随机发明互相矛盾的新特征；需要补充女性化描写时，应使用与已建立设定兼容的细节，并保持后续复用。
16. 百合向关系推进必须承接前文。暧昧、信任、依赖、吃醋、保护欲、亲密距离、旁人态度和称谓变化要符合当前剧情阶段，不能突然重置或跳跃。
17. 女性化细节应覆盖正文中与主角有关的视线、评价、互动距离和社会称呼；标题仅按第 3 条的明确条件修改。新增内容必须服务当前剧情，不得破坏原文战力、伏笔、人物性格和逻辑。
18. 输入可能是完整批次，也可能是并发分片；必须一次性改写当前输入中实际出现的全部章节，不要逐章分开回答。
19. 每章必须以输入中对应的 `<<<YURI_REWRITE_CHAPTER_START ...>>>` 开始标记开头，并以对应的 `<<<YURI_REWRITE_CHAPTER_END ...>>>` 结束标记结尾；marker 中的 index 和 id 必须逐字复制，不得省略、改写或自行生成。
20. 只输出当前输入章节的边界标记、改写后标题和正文，不要解释、不要 Markdown 包裹，不要输出当前输入之外的章节。不要在正文末尾附加“标题：”“正文：”等格式标签。
